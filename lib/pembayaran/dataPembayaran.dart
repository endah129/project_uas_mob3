...
import 'package:mob3_login_022_endah/pembayaran/inputPembayaran.dart';
import 'package:intl/intl.dart'; // Untuk format tanggal

class DataPembayaranScreen extends StatefulWidget {
  const DataPembayaranScreen({super.key});

  @override
  _DataPembayaranScreenState createState() => _DataPembayaranScreenState();
}

class _DataPembayaranScreenState extends State<DataPembayaranScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _deletePembayaran(String id) async {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        actions: [
          TextButton(
            onPressed: () {
              FirebaseFirestore.instance.collection('pembayaran').doc(id).delete().then((value) {
                Navigator.of(context).pop();
              });
            },
            child: Text('Hapus'),
                      ...
                      child: filteredData == null || filteredData.isEmpty
                          ...
                          : ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final tanggalPembayaran = memberData['tanggalPembayaran'] ?? '';
                                final jumlahPembayaran = memberData['jumlahPembayaran'] ?? '';

                                // Format tanggal
                                String formattedTanggalPembayaran = '';
                                if (tanggalPembayaran.isNotEmpty) {
                                  formattedTanggalPembayaran = DateFormat('dd MMM yyyy').format(DateTime.parse(tanggalPembayaran));
                                }

                                double? jumlah;
                                if (jumlahPembayaran != null) {
                                  if (jumlahPembayaran is String) {
                                    // Menghapus semua karakter non-digit (termasuk titik) sebelum mencoba parse
                                    String sanitized = jumlahPembayaran.replaceAll(RegExp(r'[^0-9]'), '');
                                    jumlah = double.tryParse(sanitized);
                                  } else if (jumlahPembayaran is num) {
                                    jumlah = double.tryParse(jumlahPembayaran.toString());
                                  }
                                }

                                if (jumlah != null) {
                                  print(jumlah.isNegative); // Mengecek apakah angka negatif
                                } else {
